...
def Einfugen(wpd, path):
    ...
    print('INSTRUCCIONS:')
    print("Per l'estabilitat del diccionari, s'han d'escriure els nous adjectius o adverbs\n"+
        "tal i com s'indica a continuació:\n\tParaula, Significat, Tipus (Adjektiv o Adverb), Lliçó\n"+
    ...
    if n != 0:
        while counter < n:
            ...
            while len(Worter.split(',')) != 4:
                print('Format incorrecte, havia de ser: Paraula, Significat, Tipus, Lliçó ')
                Worter = input()
            wpd.loc[wpd.shape[0]+counter+1] = Worter.replace(', ',',').split(',')
            print('Paraula registrada! '+['','Següent:\t'][counter+1==n])
            counter += 1
        wpd = wpd.sort_values(by='Wort').reset_index(drop=True)            
        wpd.to_csv(path, index = False)
    pass

def Runde(wort, losung, aktion = None):
    '''This function defines a round and returns the points obtained. Guessing is penalized
    by -1 points.'''
    if aktion:
        wort,losung = losung,wort
        print('Wie sagt man «{}» auf Deutsch?'.format(wort), end = ' ')
    else:
        print('Was bedeutet «{}»?'.format(wort), end = ' ')
    a = 2
    points = 0
    while a != 0:
        auswahl = input().lower()
...
def Spiel(wpd, aktion):
    ...
    #Use the specific part of the database we are interested in
    if aktion == 'a':
        wpd = wpd[wpd.Type=='adjektiv']
    elif aktion == 'd':
        wpd = wpd[wpd.Type=='adverbien']
    ...
    for i in range(n):
        ...
        entry = games.iloc[i]
        if aktion != 'b':
            points = Runde(entry['Wort'], entry['Bedeutung'], aktion)
        else:
            points = Runde(entry['Wort'], entry['Bedeutung'])
        score += points
...
def Worterbuch(wpd, aktion):
    ...
    if aktion == 's':
        ...
        data = wpd.loc[wpd.Wort == Wort].values
        if data.size==0:
            try:
            ...
            data = data[0]
        print()
        print(template.format('Type','Wort','Bedeutung'))
        print('='*46)
        print(template.format(data[2][:3], data[0], data[1]))
    else:
        print(template.format('Type','Wort','Bedeutung'))
        print('='*46)
...
def HauptFunktion():
    ...
    while True:
        try:
            ...
            path = input('Schreiben Sie die absolut Adresse von "adj_adv.csv" bitte: \t')
    ...
    while True:
        ...
        if aktion == 'v':
            ...
            if aktion == 's':
                while True:
                    aktion2 = input('Schreiben Sie: A - Adjectiv | D - Adverbien | '
